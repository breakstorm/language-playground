function solution2(money, company, investTable){
    if(company <= 0){
        for(var k = 0; k < flagTable.length; k++){
            temp += flagTable[k]
        }
    ...
    for(var i = 0; i <= money; i++){
        flagTable[company-1] = investTable[i][company-1];
        count++
        //console.log("투자금액 : "+(i)+" / 회사 : "+company);
        solution2(money-i, company-1, investTable)
    }
    console.log("--- max : "+max+"---");
}
...
solution2(money, company, investTable);
